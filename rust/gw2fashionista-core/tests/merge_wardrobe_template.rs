...
mod tests {
    ...
    const PEEKABOO_TEMPLATE: &str = "D7UfzTMeBQYA4gQGAJ4AHgUGAB4FAQCsAAYABgAeBQEANSgBAAYAHgUBAMkDHgUGAAEAAQDVAAYAHgUeBQEAoRYeBQYAAQABADIAAQABAAEAAQBoEqAPFCovKj8SAAD/fg==";

    #[test]
    fn test_filter_zizi() {
        let chat_link = &ChatLink::try_from(PEEKABOO_TEMPLATE).unwrap();
        let ChatLink::WardrobeTemplate(base_template) = chat_link else {
           panic!("Expected WardrobeTemplate, got {chat_link:?}");
        };

        let chat_link = &ChatLink::try_from(ZIZI_ARMOR_TEMPLATE).unwrap();
        ...
        for (slot_type, slot) in &merged {
            match slot_type {
                SlotType::Backpack | SlotType::Chest | SlotType::Shoes | SlotType::Gloves | SlotType::Head | SlotType::Legs | SlotType::Shoulders => {
        ...
        for (slot_type, slot) in &merged {
            match slot_type {
                SlotType::Backpack | SlotType::Chest | SlotType::Shoes | SlotType::Gloves | SlotType::Head | SlotType::Legs | SlotType::Shoulders => {
                    ...
                    assert_eq!(slot.is_visible(), armor_template.get_slot(&slot_type).is_visible());
        ...
        for (slot_type, slot) in &merged {
            match slot_type {
                SlotType::Backpack | SlotType::Chest | SlotType::Shoes | SlotType::Gloves | SlotType::Head | SlotType::Legs | SlotType::Shoulders => {
                    ...
                    assert_eq!(slot.is_visible(), base_template.get_slot(&slot_type).is_visible());
